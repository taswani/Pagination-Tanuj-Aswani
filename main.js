...
let numElements = 10; //setting up a variable to display a certain amount of students per page.
...
function showPage(pageNumber, sList) {
  ...
  for (let i = 0; i < sList.length; i++){
    if ((numElements * (pageNumber - 1)) <= i && i <= ((pageNumber * numElements) - 1)) {
      sList[i].show();
    }
  };
};
...
function appendPageLinks(sList) {
  ...
  let pageNums = Math.ceil(sList.length / numElements); //variable to hold the amount of pagination links needed.
